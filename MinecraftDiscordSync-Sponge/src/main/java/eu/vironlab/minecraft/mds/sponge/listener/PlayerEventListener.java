...
public class PlayerEventListener {
	...
	@Listener
	public void onEvent(ClientConnectionEvent.Join event) {
		if(event == null) return;
		if(!SpongeMinecraftDiscordSync.enabled) return;
		if(!SpongeMinecraftDiscordSync.getInstance().getConfig().getBoolean("plugin.enable_playerevents", false)) return;
		...
		String messageFormat = SpongeMinecraftDiscordSync.getInstance().getConfig().getString("format.event_playerjoin", "%playername% Joined the Minecraft Server");
		...
		String message = messageFormat.replace("%pluginprefix%", pluginPrefix)
				.replace("%playername%", ChatColor.stripColorCodes(event.getTargetEntity().getName()));
	...
	@Listener
	public void onEvent(ClientConnectionEvent.Disconnect event) {
		if(event == null) return;
		if(!SpongeMinecraftDiscordSync.enabled) return;
		if(!SpongeMinecraftDiscordSync.getInstance().getConfig().getBoolean("plugin.enable_playerevents", false)) return;
		...
		boolean useEmbed = SpongeMinecraftDiscordSync.getInstance().getConfig().getBoolean("format.use_embeds", false);
		String messageFormat = SpongeMinecraftDiscordSync.getInstance().getConfig().getString("format.event_playerquit", "%playername% Left the Minecraft Server");
		String pluginPrefix = SpongeMinecraftDiscordSync.getInstance().getConfig().getString("plugin.prefix", "MinecraftDiscordSync");
		String message = messageFormat.replace("%pluginprefix%", pluginPrefix)
				.replace("%playername%", ChatColor.stripColorCodes(event.getTargetEntity().getName()));
	...
	private Color getColor(String colFor, String defaultColor) {
		String hexString = SpongeMinecraftDiscordSync.getInstance().getConfig().getString("embed.colors." + colFor, defaultColor);
