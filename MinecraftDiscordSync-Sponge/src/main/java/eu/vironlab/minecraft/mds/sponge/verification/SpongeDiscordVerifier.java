...
package eu.vironlab.minecraft.mds.sponge.verification;
...
public class SpongeDiscordVerifier implements IVerificationProvider {

	@Override
	public boolean verifyUser(UUID minecraftUUID, String discordID, boolean setVerified) {
		IStorageProvider storageProvider = SpongeMinecraftDiscordSync.getInstance().getPluginAPI().getStorageProvider();

		JDA bot = SpongeMinecraftDiscordSync.getInstance().getPluginAPI().getDiscordBot().getJDA();
		IPermissionProvider permissionProvider = SpongeMinecraftDiscordSync.getInstance().getPluginAPI()
				.getPermissionProvider();

		Guild guild = null;
		try {
			guild = bot.getGuildById(SpongeMinecraftDiscordSync.getInstance().getConfig().getString("guild.id", "123"));
		} catch (Exception e) {
			return false;
		}
		if (guild == null)
		...
		try {
			...
			try {
				setRoles(SpongeMinecraftDiscordSync.getInstance().getConfig()
						.getStringList("verification." + primaryGroupName + ".roles"), discordID, guild);
			} catch (Exception e) {
			}
			if (SpongeMinecraftDiscordSync.getInstance().getConfig()
					.exists("verification." + primaryGroupName + ".includes")) {
				for (String group : SpongeMinecraftDiscordSync.getInstance().getConfig()
