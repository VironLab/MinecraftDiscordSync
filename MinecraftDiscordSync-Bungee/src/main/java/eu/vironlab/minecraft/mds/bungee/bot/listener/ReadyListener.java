...
package eu.vironlab.minecraft.mds.bungee.bot.listener;

import java.util.concurrent.TimeUnit;

import eu.vironlab.minecraft.mds.bungee.BungeeMinecraftDiscordSync;
import eu.vironlab.minecraft.mds.bungee.server.BungeeServerUtil;
...
public class ReadyListener extends ListenerAdapter {
	...
	public void onReady(ReadyEvent event) {
		String status = BungeeMinecraftDiscordSync.getInstance().getConfig().getString("bot.activity.message",
				"VironLab MinecraftDiscordSync");
		String statusType = BungeeMinecraftDiscordSync.getInstance().getConfig().getString("bot.activity.type",
				"playing");
		String onlineType = BungeeMinecraftDiscordSync.getInstance().getConfig().getString("bot.activity.status",
				"online");
		...
		if (BungeeMinecraftDiscordSync.getInstance().getConfig().getBoolean("onlinecounter.enabled", false)) {
			ProxyServer.getInstance().getScheduler().schedule(BungeeMinecraftDiscordSync.getInstance(), () -> {
				VoiceChannel channel = BungeeMinecraftDiscordSync.getInstance().getPluginAPI().getDiscordBot().getJDA()
						.getGuildById(BungeeMinecraftDiscordSync.getInstance().getConfig().getString("guild.id", ""))
						.getVoiceChannelById(BungeeMinecraftDiscordSync.getInstance().getConfig()
								.getString("onlinecounter.counterchannel", ""));
				if (channel != null) {
					String nameFormat = BungeeMinecraftDiscordSync.getInstance().getConfig()
