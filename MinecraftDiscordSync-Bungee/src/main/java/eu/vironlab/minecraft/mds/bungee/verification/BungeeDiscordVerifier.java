...
package eu.vironlab.minecraft.mds.bungee.verification;
...
public class BungeeDiscordVerifier implements IVerificationProvider {

	@Override
	public boolean verifyUser(UUID minecraftUUID, String discordID, boolean setVerified) {
		IStorageProvider storageProvider = BungeeMinecraftDiscordSync.getInstance().getPluginAPI().getStorageProvider();

		JDA bot = BungeeMinecraftDiscordSync.getInstance().getPluginAPI().getDiscordBot().getJDA();
		IPermissionProvider permissionProvider = BungeeMinecraftDiscordSync.getInstance().getPluginAPI()
				.getPermissionProvider();

		Guild guild = null;
		try {
			guild = bot.getGuildById(BungeeMinecraftDiscordSync.getInstance().getConfig().getString("guild.id", "123"));
		} catch (Exception e) {
			return false;
		}
		if (guild == null)
		...
		try {
			...
			try {
				setRoles(BungeeMinecraftDiscordSync.getInstance().getConfig()
						.getStringList("verification." + primaryGroupName + ".roles"), discordID, guild);
			} catch (Exception e) {
			}
			if (BungeeMinecraftDiscordSync.getInstance().getConfig()
					.contains("verification." + primaryGroupName + ".includes")) {
				for (String group : BungeeMinecraftDiscordSync.getInstance().getConfig()
