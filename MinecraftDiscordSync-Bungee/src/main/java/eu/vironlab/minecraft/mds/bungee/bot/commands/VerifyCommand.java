...
package eu.vironlab.minecraft.mds.bungee.bot.commands;
...
public class VerifyCommand extends DiscordCommand {
	...
	public boolean execute(CommandData data) {
		...
		if(data.getArgs().size() < 1) {
			data.reply(BungeeMinecraftDiscordSync.getInstance().getPluginMessages().translate("command.discord.verify.missingcode"));
			return false;
		}
		...
		if(!VerificationQueue.verificationProcessing.containsValue(code)) {
			data.reply(BungeeMinecraftDiscordSync.getInstance().getPluginMessages().translate("command.discord.verify.wrongcode"));
			return false;
		}
		...
		if(BungeeMinecraftDiscordSync.getInstance().getPluginAPI().getVerificationProvider().verifyUser(playerUuid, data.getAuthor().getId()))
			data.reply(BungeeMinecraftDiscordSync.getInstance().getPluginMessages().translate("command.discord.verify.success"));
		else 
			data.reply(BungeeMinecraftDiscordSync.getInstance().getPluginMessages().translate("command.discord.verify.error"));
		return false;
	}
