...
package eu.vironlab.minecraft.mds.bukkit.verification;
...
public class BukkitDiscordVerifier implements IVerificationProvider {

	@Override
	public boolean verifyUser(UUID minecraftUUID, String discordID, boolean setVerified) {
		IStorageProvider storageProvider = BukkitMinecraftDiscordSync.getInstance().getPluginAPI().getStorageProvider();

		JDA bot = BukkitMinecraftDiscordSync.getInstance().getPluginAPI().getDiscordBot().getJDA();
		IPermissionProvider permissionProvider = BukkitMinecraftDiscordSync.getInstance().getPluginAPI()
				.getPermissionProvider();

		Guild guild = null;
		try {
			guild = bot.getGuildById(BukkitMinecraftDiscordSync.getInstance().getConfig().getString("guild.id", "123"));
		} catch (Exception e) {
			return false;
		}
		if (guild == null)
		...
		try {
			...
			try {
				setRoles(BukkitMinecraftDiscordSync.getInstance().getConfig()
						.getStringList("verification." + primaryGroupName + ".roles"), discordID, guild);
			} catch (Exception e) {
			}
			if (BukkitMinecraftDiscordSync.getInstance().getConfig()
					.contains("verification." + primaryGroupName + ".includes")) {
				for (String group : BukkitMinecraftDiscordSync.getInstance().getConfig()
