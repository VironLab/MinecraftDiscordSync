...
package eu.vironlab.minecraft.mds.nukkit.bot;

import java.util.ArrayList;
import java.util.List;

import eu.vironlab.minecraft.mds.nukkit.NukkitMinecraftDiscordSync;
import eu.vironlab.minecraft.mds.nukkit.bot.commands.*;
import eu.vironlab.minecraft.mds.nukkit.bot.listener.*;
...
public class DiscordSyncBot extends DiscordBot {
	...
	public DiscordSyncBot(IPluginLogger pluginLogger, String token, String commandPrefix) {
		...
		if(NukkitMinecraftDiscordSync.getInstance().getConfig().getBoolean("plugin.commands_enabled.discord.online", false))
			defaultCommands.add(new OnlineCommand());
		if(NukkitMinecraftDiscordSync.getInstance().getConfig().getBoolean("plugin.commands_enabled.discord.playerlist", false))
			defaultCommands.add(new PlayerListCommand());
		if(NukkitMinecraftDiscordSync.getInstance().getConfig().getBoolean("plugin.commands_enabled.discord.serverinfo", false))
			defaultCommands.add(new ServerInfoCommand());	
		if(NukkitMinecraftDiscordSync.getInstance().getConfig().getBoolean("plugin.commands_enabled.discord.verify", false))
