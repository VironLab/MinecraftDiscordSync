...
public class VerifyCommand extends DiscordCommand {
	...
	public boolean execute(CommandData data) {
		...
		if(data.getArgs().size() < 1) {
			data.reply(NukkitMinecraftDiscordSync.getInstance().getPluginMessages().translate("command.discord.verify.missingcode"));
			return false;
		}
		...
		if(!VerificationQueue.verificationProcessing.containsValue(code)) {
			data.reply(NukkitMinecraftDiscordSync.getInstance().getPluginMessages().translate("command.discord.verify.wrongcode"));
			return false;
		}
		...
		if(NukkitMinecraftDiscordSync.getInstance().getPluginAPI().getVerificationProvider().verifyUser(playerUuid, data.getAuthor().getId()))
			data.reply(NukkitMinecraftDiscordSync.getInstance().getPluginMessages().translate("command.discord.verify.success"));
		else 
			data.reply(NukkitMinecraftDiscordSync.getInstance().getPluginMessages().translate("command.discord.verify.error"));
		return false;
	}
