...
package eu.vironlab.minecraft.mds.nukkit.verification;
...
public class NukkitDiscordVerifier implements IVerificationProvider {

	@Override
	public boolean verifyUser(UUID minecraftUUID, String discordID, boolean setVerified) {
		IStorageProvider storageProvider = NukkitMinecraftDiscordSync.getInstance().getPluginAPI().getStorageProvider();

		JDA bot = NukkitMinecraftDiscordSync.getInstance().getPluginAPI().getDiscordBot().getJDA();
		IPermissionProvider permissionProvider = NukkitMinecraftDiscordSync.getInstance().getPluginAPI()
				.getPermissionProvider();

		Guild guild = null;
		try {
			guild = bot.getGuildById(NukkitMinecraftDiscordSync.getInstance().getConfig().getString("guild.id", "123"));
		} catch (Exception e) {
			return false;
		}
		if (guild == null)
		...
		try {
			...
			try {
				setRoles(NukkitMinecraftDiscordSync.getInstance().getConfig()
						.getStringList("verification." + primaryGroupName + ".roles"), discordID, guild);
			} catch (Exception e) {
			}
			if (NukkitMinecraftDiscordSync.getInstance().getConfig()
					.exists("verification." + primaryGroupName + ".includes")) {
				for (String group : NukkitMinecraftDiscordSync.getInstance().getConfig()
