...
public class Config extends LinkedHashMap<String, Object> {
	...
	private static final long serialVersionUID = 7158503092555525259L;
	...
	public boolean load(String file, int type, ConfigSection defaultMap) {
		...
		if (!this.file.exists()) {
			try {
				...
				VelocityMinecraftDiscordSync.getInstance().getLogger().error("Could not create Config " + this.file.toString(), e);
			}
			...
			if (this.correct) {
				...
				try {
					...
					VelocityMinecraftDiscordSync.getInstance().getLogger().error(e.getMessage() + e.getStackTrace().toString());
				}
	...
	public boolean load(InputStream inputStream) {
		...
		if (this.correct) {
			...
			try {
				...
				VelocityMinecraftDiscordSync.getInstance().getLogger().error("",e);
				return false;
			}
			this.parseContent(content);
		}
		...
		for (final String line : content.split("\n")) {
			if (Pattern.compile("[a-zA-Z0-9\\-_.]*+=+[^\\r\\n]*").matcher(line).matches()) {
				...
				if (this.config.containsKey(key)) {
					VelocityMinecraftDiscordSync.getInstance().getLogger().debug("[Config] Repeated property " + key + " on file " + this.file.toString());
				}
				switch (valueLower) {
				case "on":
				case "true":
