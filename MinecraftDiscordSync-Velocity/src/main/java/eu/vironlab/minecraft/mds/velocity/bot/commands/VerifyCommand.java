...
public class VerifyCommand extends DiscordCommand {
	...
	public boolean execute(CommandData data) {
		...
		if(!VerificationQueue.verificationProcessing.containsValue(code)) {
			data.reply(VelocityMinecraftDiscordSync.getInstance().getPluginMessages().translate("command.discord.verify.wrongcode"));
			return false;
		}
		...
		if(VelocityMinecraftDiscordSync.getInstance().getPluginAPI().getVerificationProvider().verifyUser(playerUuid, data.getAuthor().getId()))
			data.reply(VelocityMinecraftDiscordSync.getInstance().getPluginMessages().translate("command.discord.verify.success"));
		else 
			data.reply(VelocityMinecraftDiscordSync.getInstance().getPluginMessages().translate("command.discord.verify.error"));
		return false;
	}
