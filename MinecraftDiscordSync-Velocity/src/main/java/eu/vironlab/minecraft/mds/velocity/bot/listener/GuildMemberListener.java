...
public class GuildMemberListener extends ListenerAdapter {
	...
	public void onGuildMemberJoin(GuildMemberJoinEvent event) {
		...
		String messageFormat = VelocityMinecraftDiscordSync.getInstance().getConfig().getString("format.memberjoin_discordserver", "&a%username%#%discriminator% joined the Discordserver &e%servername%");
		String message = messageFormat.replace("%username%", username).replace("%discriminator%", discrim).replace("%servername%", servername);
		
		VelocityServerUtil.broadcastMessage(message);
	...
	public void onGuildMemberRemove(GuildMemberRemoveEvent event) {
		...
		String discrim = event.getMember().getUser().getDiscriminator();
		String servername = event.getGuild().getName();
		String messageFormat = VelocityMinecraftDiscordSync.getInstance().getConfig().getString("format.memberremove_discordserver", "&c%username%#%discriminator% left the Discordserver &e%servername%");
		String message = messageFormat.replace("%username%", username).replace("%discriminator%", discrim).replace("%servername%", servername);
