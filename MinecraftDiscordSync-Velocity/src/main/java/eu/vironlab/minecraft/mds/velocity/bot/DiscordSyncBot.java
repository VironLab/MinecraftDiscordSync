...
package eu.vironlab.minecraft.mds.velocity.bot;

import java.util.ArrayList;
import java.util.List;

import eu.vironlab.minecraft.mds.velocity.VelocityMinecraftDiscordSync;
import eu.vironlab.minecraft.mds.velocity.bot.commands.*;
import eu.vironlab.minecraft.mds.velocity.bot.listener.*;
...
public class DiscordSyncBot extends DiscordBot {
	...
	public DiscordSyncBot(IPluginLogger pluginLogger, String token, String commandPrefix) {
		...
		if(VelocityMinecraftDiscordSync.getInstance().getConfig().getBoolean("plugin.commands_enabled.discord.online", false))
			defaultCommands.add(new OnlineCommand());
		if(VelocityMinecraftDiscordSync.getInstance().getConfig().getBoolean("plugin.commands_enabled.discord.playerlist", false))
			defaultCommands.add(new PlayerListCommand());
		if(VelocityMinecraftDiscordSync.getInstance().getConfig().getBoolean("plugin.commands_enabled.discord.serverinfo", false))
			defaultCommands.add(new ServerInfoCommand());	
		if(VelocityMinecraftDiscordSync.getInstance().getConfig().getBoolean("plugin.commands_enabled.discord.verify", false))
